...
export default function HomePage() {
  ...
  const useTemplate = (templateType) => {
    router.push(`/templates/${templateType}`)
  }
        ...
        <div className="grid md:grid-cols-3 gap-6 max-w-4xl mx-auto">
          {templates.map((template, index) => (
            <Card
              ...
              onClick={() => router.push(`/templates/${template.title.toLowerCase().replace(" ", "-")}`)}
            >
...
<p className="text-slate-600 text-sm text-center">
  © {new Date().getFullYear()} All rights reserved by <strong>Xyndrix</strong>. <br />
  Developed by <strong>Xyndrix Software Company</strong>.
</p>
        </div>
      </footer>
    </div>
